...
module.exports = {
  list: () => {
    ...
    const statusPromises = Object.keys(externalProcesses).map((name) => {
      if (externalProcesses[name].pm2proc !== false) {
        ...
        return externalProcesses[name].status().catch((err) => {
          console.error(
            `error getting status of external process "${name}":`,
            err,
          );
    ...
    return Promise.all([pm2wrapper.list(), ...statusPromises]).then(
      ([list, ...statuses]) => {
  ...
  describe: (id) => {
    const externalProcesses = utils.getExternalProcesses();
    if (externalProcesses[id].pm2proc === false) {
      return externalProcesses[id]
        .describe()
        .catch((err) => {
          console.error(
            `error getting described details of external process "${id}":`,
            err,
          );
          return [];
        })
