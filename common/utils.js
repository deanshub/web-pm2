      ...
      if (config.processes !== undefined && Array.isArray(config.processes)) {
        promises = config.processes.map((process) => {
          ...
          if (existingProcesses[processName] !== undefined) {
            if (
              existingProcesses[processName].pm2_env &&
              existingProcesses[processName].pm2_env.status !== 'online'
            ) {
...
const loadExtraProcesses = () => {
  return Q.nfcall(fs.readdir, path.join(__dirname, '..', EXTRA_PROCESSES_DIR), {
    encoding: 'utf-8',
  })
    .then((files) => {
...
module.exports.getExternalProcesses = () => {
  return externalProcesses;
};
