...
app.get('/api/operations/showlog/:id/:logname', (req, res) => {
  ...
  processController
    .describe(req.params.id)
    .then((procsDetails) => {
      const logs = procsDetails.logsPaths.filter(
        (proc) => proc.name === logname,
      );
      if (logs.length > 0) {
...
app.get('/api/operations/configuration/:id', (req, res) => {
  ...
  processController
    .getConfigurations(req.params.id)
    .then((configurationDetails) => {
...
app.post('/api/operations/configuration/:id', (req, res) => {
  ...
  processController
    .setConfigurations(req.params.id, req.body.configurations)
    .then((configurationDetails) => {
      res.json(configurationDetails);
    })
    .catch((err) => {
